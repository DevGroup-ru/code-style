...
SELECT DISTINCT
    `id`, 
    `name`,
    `parent_id` -- если мало в одну строчку
FROM
    `table_name` `tn`
JOIN
    `join_table` `jt`
...
В коде представлений и шаблонов необходимо использовать [альткрнативный синтаксис](http://php.net/manual/ru/control-structures.alternative-syntax.php) управляющих структур. Использование стандартных допускается только в верхнем блоке файла до любого вывода html.
